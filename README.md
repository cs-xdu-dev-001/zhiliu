...
“内容质量”页面会记录Hermes每次发布中的过滤、去重、接受和写入决策，保留来源、摘要、原始链接与处理理由。被过滤的内容可在确认后恢复写入，恢复操作会保留原决策记录，便于追溯。

“订阅与任务”中的“订阅健康”按近30天汇总每个订阅的执行次数、成功率、连续失败、平均耗时和产出量。网络超时或Hermes暂时不可用时，任务会自动重试最多2次；配置错误等非瞬时错误会直接标记失败。
