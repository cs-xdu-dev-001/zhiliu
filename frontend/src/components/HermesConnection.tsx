...
export function HermesConnection() {
  ...
  return <section className="hermes-connection" aria-label="Hermes连接">
    <div className="hermes-summary"><div><h2>Hermes连接</h2>{query.isPending ? <p>正在检查连接…</p> : query.isError ? <p role="alert">连接信息加载失败，请重试</p> : <p className={`hermes-status ${d?.status ?? "error"}`}>{d ? `${labels[d.status]}：${d.message}` : "尚未配置Hermes服务"}</p>}</div>
