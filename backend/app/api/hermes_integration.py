...
async def get_hermes(db: Session = Depends(get_db), settings: Settings = Depends(get_settings)):
    return HermesIntegrationService(db, settings).response()

@router.put("", response_model=HermesConnectionResponse)
async def put_hermes(payload: HermesConnectionUpdate, db: Session = Depends(get_db), settings: Settings = Depends(get_settings)):
    try:
...
async def test_hermes(db: Session = Depends(get_db), settings: Settings = Depends(get_settings)):
